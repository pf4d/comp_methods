...
int main()
{
	...
	for (unsigned int i = 0; i < n_a.size(); i++)
	{
		...
		double       g_r = -1.0;         // outward flux of u at x = x_r
		double       u_l =  0.0;         // left essential boundary condition
		double       f_x =  0.0;         // constant source function
		...
		ofstream x_file ("../data/b/x_" + to_string(n) + ".txt");
		ofstream u_file ("../data/b/u_" + to_string(n) + ".txt");
	...
	// output the vectors to files for later plotting :
	ofstream n_file ("../data/b/n.txt");
	ofstream e_file ("../data/b/e.txt");
	ofstream i_file ("../data/b/i.txt");
	ofstream t_file ("../data/b/t.txt");
