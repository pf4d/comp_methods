...
ls_a = [':', '-.', '--', '-']
n_a  = np.loadtxt(data_dir + "a/n.txt").astype('int')
...
for n, ls in zip(n_a, ls_a):
	...
	x = np.loadtxt(data_dir + "a/x_%i.txt" % n)
	u = np.loadtxt(data_dir + "a/u_%i.txt" % n)
...
plt.savefig("../images/prob_2_part_a.pdf")
plt.close()
